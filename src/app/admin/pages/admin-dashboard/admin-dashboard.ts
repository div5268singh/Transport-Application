...
export class AdminDashboard implements OnInit {
  ...
  protected editingCommitmentText = '';

  // Search-engine title and description shown for each public page.
  private static readonly pageSeoFields: Array<{ label: string; titleKey: string; descriptionKey: string }> = [
    { label: 'Home', titleKey: 'home.seoTitle', descriptionKey: 'home.seoDescription' },
    { label: 'About', titleKey: 'about.seoTitle', descriptionKey: 'about.seoDescription' },
    { label: 'Services', titleKey: 'services.seoTitle', descriptionKey: 'services.seoDescription' },
    { label: 'Clients', titleKey: 'clients.seoTitle', descriptionKey: 'clients.seoDescription' },
  ...
  addCommitment(): void {
    ...
    const nextConfig = this.appConfigService.getEditableConfig();
    nextConfig.about.operationalFootprint.push(this.newCommitmentText.trim());
    this.newCommitmentText = '';
    this.persist(nextConfig, 'Operational commitment added successfully.');
  }

  removeCommitment(index: number): void {
    if (this.commitments.length <= 1) {
      this.contentStatus = 'At least one commitment must remain. Add a replacement before removing this one.';
      return;
    }

    const nextConfig = this.appConfigService.getEditableConfig();
    nextConfig.about.operationalFootprint.splice(index, 1);
    if (this.editingCommitmentIndex === index) {
      this.cancelCommitmentEdit();
    }
    this.persist(nextConfig, 'Operational commitment removed successfully.');
  }

  startCommitmentEdit(index: number): void {
    const commitment = this.commitments[index];
    if (commitment === undefined) {
      this.contentStatus = 'Selected commitment not found.';
      return;
    }

    this.editingCommitmentIndex = index;
    this.editingCommitmentText = commitment;
  }

  saveCommitmentEdit(): void {
    if (this.editingCommitmentIndex < 0) {
      this.contentStatus = 'No commitment selected for editing.';
      return;
    }
    if (!this.editingCommitmentText.trim()) {
      this.contentStatus = 'Commitment text is required.';
      return;
    }

    const nextConfig = this.appConfigService.getEditableConfig();
    nextConfig.about.operationalFootprint[this.editingCommitmentIndex] = this.editingCommitmentText.trim();
    this.persist(nextConfig, 'Operational commitment updated successfully.');
    this.cancelCommitmentEdit();
  }

  cancelCommitmentEdit(): void {
    this.editingCommitmentIndex = -1;
  ...
  savePageSeo(index: number): void {
    ...
    if (!entry.title.trim() || !entry.description.trim()) {
      this.contentStatus = `SEO title and description are required for ${entry.label}.`;
      return;
    }

    const nextConfig = this.appConfigService.getEditableConfig();
    nextConfig.content[entry.titleKey] = entry.title.trim();
    nextConfig.content[entry.descriptionKey] = entry.description.trim();
    this.persist(nextConfig, `${entry.label} SEO details updated successfully.`);
  }

  async resetContentToDefault(): Promise<void> {
    try {
      await this.appConfigService.clearConfigOverride();
      this.refreshConfigState();
      this.previewBannerIndex = 0;
